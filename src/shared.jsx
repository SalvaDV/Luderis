...
export const Avatar=({letra,size=38,img,radius,color})=>{
  ...
  // color elegido por el usuario (avatar_color) tiene prioridad sobre el degradado por letra
  const bg=color||`linear-gradient(135deg,${from},${to})`;
  ...
  return<div title={typeof letra==='string'&&letra.length>1?letra:undefined} style={{width:size,height:size,borderRadius:br,background:bg,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:size*0.38,flexShrink:0,boxShadow:"0 2px 8px rgba(0,0,0,.15)",letterSpacing:"-.5px"}}>{(letra||"?").toUpperCase()}</div>;
};
...
const LEGAL_SECTIONS=[
  ...
  {title:"3. Registro y cuenta",body:"Podés usar la plataforma si tenés 18 años o más; o entre 13 y 17 años con autorización expresa de un tutor legal (menores de 13 no pueden registrarse). Sos responsable de mantener la confidencialidad de tu contraseña y de toda actividad que ocurra en tu cuenta. Notificá inmediatamente cualquier uso no autorizado a contacto@luderis.com."},
  ...
  {title:"6. Pagos y comisiones",body:"Los pagos procesados a través de MercadoPago están sujetos a los términos de dicho servicio. Luderis cobra una comisión variable sobre cada transacción realizada dentro de la plataforma; el porcentaje vigente se informa antes de confirmar cualquier pago o publicación. Los precios publicados son en la moneda indicada por el docente."},
