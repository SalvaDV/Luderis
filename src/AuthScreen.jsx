...
const CONTAINER={animate:{transition:{staggerChildren:0.11,delayChildren:0.4}}};
const FEATURE_ITEM={
  initial:{x:-28,opacity:0},
  animate:{x:0,opacity:1,transition:{type:"spring",stiffness:110,damping:18}}
};
const FORM_VARIANT={
...
function AuthScreen({onLogin}){
        ...
        if(r.access_token){
          ...
          try{
            ...
            if(refCode&&uid){
              ...
              if(referidorId){
                await sb.db("referidos","POST",{referidor_id:referidorId,referido_id:uid,referido_email:email,estado:"pendiente"},r.access_token,"return=minimal").catch(e=>logError("registrar referido",e));
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",fontFamily:FONT,background:"#F6F9FF"}}>
      ...
      <div className="auth-left" style={{flex:"0 0 400px",background:"linear-gradient(160deg,#0A2A5E 0%,#1A6ED8 55%,#2EC4A0 100%)",display:"flex",flexDirection:"column",padding:"48px 40px",position:"relative",overflow:"hidden",justifyContent:"space-between"}}>

        {/* Orbes animados */}
        {[
          {w:340,h:340,top:-90,right:-90,bg:"rgba(255,255,255,.05)"},
          {w:220,h:220,bottom:30,left:-70,bg:"rgba(46,196,160,.13)"},
          {w:160,h:160,top:"38%",right:"15%",bg:"rgba(255,255,255,.04)"},
        ].map((o,i)=>(
          <motion.div key={i} custom={i} variants={ORB_VARIANTS} animate="animate"
            style={{position:"absolute",width:o.w,height:o.h,borderRadius:"50%",background:o.bg,
        ...
        <div style={{position:"relative",zIndex:1}}>
          <motion.div initial={{opacity:0,y:-16}} animate={{opacity:1,y:0}} transition={{type:"spring",stiffness:120,damping:18}}
            ...
            <motion.div
              ...
              style={{width:52,height:52,borderRadius:14,background:"rgba(255,255,255,.15)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(0,0,0,.2)"}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"24px 16px"}}>
        <motion.div variants={FORM_VARIANT} initial="initial" animate="animate" style={{width:"min(420px,100%)"}}>
          ...
          {mode!=="forgot"&&(
            <>
              <motion.button onClick={handleGoogle} disabled={loading} whileHover={{scale:1.01,boxShadow:"0 4px 16px rgba(26,110,216,.15)"}} whileTap={{scale:.98}}
                style={{width:"100%",background:"#fff",border:"1.5px solid #DDE5F5",borderRadius:12,padding:"12px 16px",fontFamily:FONT,fontSize:14,fontWeight:600,color:"#0D1F3C",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:16,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
                ...
                <>
                  ...
                  <motion.button onClick={handle} disabled={loading||(mode==="register"&&!aceptoTerminos)} whileHover={{scale:1.01,boxShadow:"0 6px 24px rgba(26,110,216,.4)"}} whileTap={{scale:.98}}
                    style={{width:"100%",background:LUD.grad,border:"none",borderRadius:20,color:"#fff",padding:"14px",fontWeight:700,fontSize:15,cursor:"pointer",fontFamily:FONT,marginBottom:14,
                      boxShadow:"0 4px 18px rgba(26,110,216,.35)",transition:"opacity .15s",
