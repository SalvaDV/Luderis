...
import { DescExpandible, InscribirseBtn, MPCheckoutBtn, RelacionadasSection, ReseñasSeccion } from "./CursoPage";
                  ...
                  <>
                    ...
                    {post.tipo==="oferta"&&!esMio&&!esAyudante&&!inscripcion&&!post.finalizado&&!post.inscripciones_cerradas&&(
                      <div style={{display:"flex",flexDirection:"column",gap:8}}>
                        {/* Selector de paquetes si existen */}
                        {(()=>{
                          let pqs=[];try{pqs=JSON.parse(post.paquetes||"[]").filter(p=>p&&p.clases>0);}catch{}
                          if(!pqs.length)return null;
                          return(
                            <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"10px 12px"}}>
                              <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:.4,marginBottom:8}}>📦 PAQUETES CON DESCUENTO</div>
                              <div style={{display:"flex",flexDirection:"column",gap:5}}>
                                {pqs.map((pq,i)=>{
                                  ...
                                  return(
                                    <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 10px",borderRadius:8,background:C.surface,border:`1px solid ${C.border}`}}>
                                      <div>
                                        <div style={{fontSize:12,fontWeight:700,color:C.text}}>{pq.nombre||`${pq.clases} clases`}
                                          {pq.descuento>0&&<span style={{marginLeft:6,fontSize:10,background:C.success+"20",color:C.success,borderRadius:20,padding:"1px 6px",fontWeight:700}}>-{pq.descuento}%</span>}
                                        </div>
                                        <div style={{fontSize:10,color:C.muted}}>${porClase.toLocaleString("es-AR")}/clase</div>
                                      </div>
                                      <div style={{fontWeight:800,color:C.success,fontSize:13}}>${total.toLocaleString("es-AR")}</div>
                                    </div>
                                  );
                                })}
                              </div>
