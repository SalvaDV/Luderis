...
const StatusBadge=({activo,finalizado,pendiente})=>{
  ...
  if(pendiente)return<span style={{fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,background:"#F5C84218",color:C.accent,border:"1px solid #F5C84233",fontFamily:FONT}}>⏳ Validación pendiente</span>;
...
function EvaluacionesFormalesConQuizzes({post,session,esMio,esAyudante,inscripcion,inscripciones,contenido,setContenido,expandedQuizzes,setExpandedQuizzes,editingQuiz,setEditingQuiz,tieneAcceso}){
  ...
  const [showCrearQuiz,setShowCrearQuiz]=useState(false);
  ...
  const totalItems=evaluaciones.length+quizzes.length;
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={{fontWeight:700,color:C.text,fontSize:14}}>
          Evaluaciones {hasAny&&<span style={{color:C.muted,fontWeight:400,fontSize:12}}>({totalItems})</span>}
        </div>
        {(esMio||esAyudante)&&(
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>{setShowCrearEv(v=>!v);setShowCrearQuiz(false);}} style={{background:"#5CA8E015",border:"1px solid #5CA8E033",borderRadius:8,color:C.info,padding:"5px 11px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>🎓 Evaluación formal</button>
            <button onClick={()=>{setShowCrearQuiz(v=>!v);setShowCrearEv(false);}} style={{background:"#C85CE015",border:"1px solid #C85CE033",borderRadius:8,color:C.purple,padding:"5px 11px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>📝 Quiz</button>
          </div>
        )}
      </div>

      {/* Formulario nueva evaluación formal */}
      ...
      {/* Formulario nuevo quiz */}
      {(esMio||esAyudante)&&showCrearQuiz&&(
        <QuizCreator publicacionId={pubId} session={session}
          onSaved={(item)=>{setContenido(prev=>[...prev,item]);setShowCrearQuiz(false);}}
          onCancel={()=>setShowCrearQuiz(false)}/>
      )}

      {loadingEv?<Spinner small/>:!hasAny?(
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"30px",textAlign:"center",color:C.muted,fontSize:13}}>
          ...
          {quizzes.map(c=>{
            ...
            return(<div key={c.id} style={{background:C.card,border:`1px solid ${C.purple}33`,borderRadius:12,overflow:"hidden"}}>
              <div onClick={()=>setExpandedQuizzes(p=>{const n=new Set(p);n.has(c.id)?n.delete(c.id):n.add(c.id);return n;})}
                ...
                <div style={{display:"flex",alignItems:"center",gap:7}}>
                  {(esMio||esAyudante)&&<button onClick={e=>{e.stopPropagation();if(setEditingQuiz)setEditingQuiz(c);}} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:7,color:C.muted,fontSize:11,padding:"2px 8px",cursor:"pointer",fontFamily:FONT}}>✎</button>}
                  {(esMio||esAyudante)&&<button onClick={e=>{e.stopPropagation();if(window.confirm("¿Eliminar este quiz?"))sb.deleteContenido(c.id,session.access_token).then(()=>setContenido(prev=>prev.filter(x=>x.id!==c.id)));}} style={{background:"none",border:"none",color:C.danger,fontSize:15,cursor:"pointer"}}>×</button>}
              ...
              {quizExpanded&&<div style={{padding:"0 15px 12px",borderTop:`1px solid ${C.border}22`}}>
                {tieneAcceso&&<SafeWrapper><QuizViewer item={c} session={session} esMio={esMio||esAyudante} esAyudante={esAyudante} onReabrir={reabrirQuiz}/></SafeWrapper>}
                {!tieneAcceso&&<div style={{fontSize:11,color:C.muted,padding:"8px 0"}}>Inscribite para acceder al examen.</div>}
              </div>}
            </div>);
          })}

          {/* Examen final siempre al final */}
          {evFinal.map(ev=>(
            <EvaluacionCard key={ev.id} ev={ev} post={post} session={session} esMio={esMio||esAyudante} inscripciones={inscripciones} inscripcion={inscripcion}
              onDelete={async()=>{await sb.deleteEvaluacion(ev.id,session.access_token).catch(()=>{});setEvaluaciones(p=>p.filter(x=>x.id!==ev.id));}}/>
          ))}
...
// UI mejorada: tipo arriba, luego preguntas concretas según formato (sin textarea JSON)
function EvaluacionCreadorMejorado({post,session,onSaved,onCancel}){
  ...
  const [generandoIA,setGenerandoIA]=useState(false);
  const [temaIA,setTemaIA]=useState("");
  // Preguntas según formato
  const [pregsMC,setPregsMC]=useState([{texto:"",opciones:["","","",""],correcta:0}]);
  ...
  const generarConIA=async()=>{
    ...
    try{
      ...
      let prompt="";
      if(evalFormato==="multiple_choice"||evalFormato==="verdadero_falso"){
        prompt=`Curso: "${post.titulo}" (${post.materia}). Tipo: ${evalTipo}. Tema: ${temaIA}.\nGenerá 4 preguntas ${evalFormato==="verdadero_falso"?"de verdadero/falso (opciones: ['Verdadero','Falso'])":"de multiple choice (4 opciones)"}.\nJSON: {"preguntas":[{"texto":"...","opciones":["..."],"correcta":0}]}`;
      }else if(evalFormato==="desarrollo"||evalFormato==="rubrica"){
        prompt=`Curso: "${post.titulo}". Tema: ${temaIA}. Generá 2 preguntas a desarrollar.\nJSON: {"preguntas":[{"texto":"...","criterios":"criterios de evaluación"}]}`;
      }else{
        prompt=`Curso: "${post.titulo}". Tema: ${temaIA}. Tipo: ${evalTipo}.\nGenerá una consigna práctica clara.\nJSON: {"consigna":"...","criterios":["criterio 1","criterio 2","criterio 3"]}`;
      }
      const res=await sb.callIA(system,prompt,600);
      const clean=res.replace(/```json|```/g,"").trim();
      const parsed=JSON.parse(clean);
      if(parsed.preguntas&&(evalFormato==="multiple_choice"||evalFormato==="verdadero_falso")){
        setPregsMC(parsed.preguntas.map(p=>({texto:p.texto||"",opciones:(p.opciones||["","",""]).slice(0,4),correcta:typeof p.correcta==="number"?p.correcta:0})));
      }else if(parsed.preguntas){
        setPregsDesarrollo(parsed.preguntas.map(p=>({texto:p.texto||"",criterios:p.criterios||""})));
      }else if(parsed.consigna){
        setConsigna(parsed.consigna);
        if(parsed.criterios)setCriterios(parsed.criterios);
      }
  ...
  const guardar=async()=>{
    ...
    try{
      let contenidoJson={};
      if(evalFormato==="multiple_choice"||evalFormato==="verdadero_falso")contenidoJson={preguntas:pregsMC};
  ...
  const esMC=["multiple_choice","verdadero_falso"].includes(evalFormato);
    ...
    <div style={{background:C.surface,border:`1px solid ${C.accent}44`,borderRadius:14,padding:18,marginBottom:14,animation:"fadeIn .15s ease"}}>
      ...
      <div style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {[["diagnostico","🔍 Diagnóstico",C.info],["checkpoint","📍 Checkpoint",C.warn],["final","🏁 Final",C.success]].map(([v,l,col])=>(
            <button key={v} onClick={()=>setEvalTipo(v)}
              style={{flex:1,padding:"8px 4px",borderRadius:9,border:`1.5px solid ${evalTipo===v?col:C.border}`,background:evalTipo===v?(col+"18"):"transparent",color:evalTipo===v?col:C.muted,fontSize:11,cursor:"pointer",fontFamily:FONT,fontWeight:evalTipo===v?700:400}}>
              {l}
            </button>
          ))}
        </div>
        {evalTipo==="final"&&<div style={{fontSize:10,color:C.muted,marginTop:5}}>ℹ️ El examen final se crea deshabilitado. Podés habilitarlo cuando el curso termine.</div>}
      </div>
        ...
        {formatosGrupos.map(grupo=>(
          <div key={grupo.label} style={{marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              {grupo.items.map(([v,l])=>(
                <button key={v} onClick={()=>setEvalFormato(v)}
                  style={{padding:"6px 10px",borderRadius:8,border:`1px solid ${evalFormato===v?C.accent:C.border}`,background:evalFormato===v?C.accentDim:"transparent",color:evalFormato===v?C.accent:C.muted,fontSize:11,cursor:"pointer",fontFamily:FONT,fontWeight:evalFormato===v?700:400}}>
                  {l}
                </button>
              ))}
            </div>
      ...
      {skills.length>0&&(
        <div style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
            {skills.map(s=>(
              <button key={s.id} onClick={()=>setEvalSkillIds(p=>p.includes(s.id)?p.filter(x=>x!==s.id):[...p,s.id])}
                style={{padding:"4px 10px",borderRadius:20,fontSize:11,cursor:"pointer",fontFamily:FONT,background:evalSkillIds.includes(s.id)?C.accent:C.surface,color:evalSkillIds.includes(s.id)?"#0D0D0D":C.muted,border:`1px solid ${evalSkillIds.includes(s.id)?C.accent:C.border}`}}>
                {s.nombre}
              </button>
            ))}
          </div>
      ...
      <div style={{background:C.card,border:"1px solid #C85CE033",borderRadius:10,padding:"10px 12px",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:7}}>
          ...
          <button onClick={generarConIA} disabled={generandoIA||!temaIA.trim()}
            style={{background:"#C85CE022",border:"1px solid #C85CE044",borderRadius:8,color:C.purple,padding:"5px 14px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700,flexShrink:0,opacity:!temaIA.trim()?0.5:1}}>
            {generandoIA?"…":"✦ Generar"}
          </button>
        </div>
      </div>
      ...
      {esMC&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <span style={{fontSize:10,color:C.muted,fontWeight:600,letterSpacing:.8}}>PREGUNTAS ({pregsMC.length})</span>
            <button onClick={()=>setPregsMC(p=>[...p,{texto:"",opciones:evalFormato==="verdadero_falso"?["Verdadero","Falso"]:["","","",""],correcta:0}])}
              style={{background:C.accentDim,border:"1px solid #F5C84233",borderRadius:7,color:C.accent,padding:"3px 9px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>+ Agregar</button>
          </div>
          {pregsMC.map((q,qi)=>(
            <div key={qi} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:12,marginBottom:8}}>
              <div style={{display:"flex",gap:7,marginBottom:8}}>
              ...
              {q.opciones.map((op,oi)=>(
                <div key={oi} style={{display:"flex",gap:7,alignItems:"center",marginBottom:5}}>
                  <button onClick={()=>setPregsMC(p=>p.map((x,i)=>i===qi?{...x,correcta:oi}:x))} style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${q.correcta===oi?C.success:C.border}`,background:q.correcta===oi?C.success:"transparent",cursor:"pointer",flexShrink:0,padding:0}}/>
                  {evalFormato==="verdadero_falso"
                    ?<span style={{color:C.text,fontSize:12}}>{op}</span>
                    :<input value={op} onChange={e=>setPregsMC(p=>p.map((x,i)=>i===qi?{...x,opciones:x.opciones.map((o,j)=>j===oi?e.target.value:o)}:x))} placeholder={`Opción ${oi+1}${q.correcta===oi?" ✓":""}`} style={{...iS,marginBottom:0,flex:1,fontSize:11}}/>
                  }
                </div>
              ))}
              <div style={{fontSize:10,color:C.muted,marginTop:3}}>● = respuesta correcta</div>
      ...
      {esDesarrollo&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            ...
            <button onClick={()=>setPregsDesarrollo(p=>[...p,{texto:"",criterios:""}])}
              style={{background:C.accentDim,border:"1px solid #F5C84233",borderRadius:7,color:C.accent,padding:"3px 9px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>+ Agregar</button>
          </div>
          {pregsDesarrollo.map((q,qi)=>(
            <div key={qi} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:12,marginBottom:8}}>
              ...
              <input value={q.criterios} onChange={e=>setPregsDesarrollo(p=>p.map((x,i)=>i===qi?{...x,criterios:e.target.value}:x))} placeholder="Criterios de evaluación (opcional)" style={{...iS,marginBottom:0,fontSize:11,color:C.muted}}/>
            </div>
          ))}
        </div>
      )}
      ...
      {esReflexion&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            ...
            <button onClick={()=>setPregsReflexion(p=>[...p,{texto:""}])}
              style={{background:C.accentDim,border:"1px solid #F5C84233",borderRadius:7,color:C.accent,padding:"3px 9px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>+ Agregar</button>
          </div>
          {pregsReflexion.map((q,qi)=>(
            <div key={qi} style={{display:"flex",gap:7,marginBottom:7}}>
              <input value={q.texto} onChange={e=>setPregsReflexion(p=>p.map((x,i)=>i===qi?{texto:e.target.value}:x))} placeholder={`Pregunta de reflexión ${qi+1}`} style={{...iS,marginBottom:0,flex:1}}/>
      ...
      {/* Consigna + criterios (práctica, código, peer review) */}
      {esPractica&&(
        <div>
          ...
          {criterios.map((c,ci)=>(
            <div key={ci} style={{display:"flex",gap:7,marginBottom:6}}>
              <input value={c} onChange={e=>setCriterios(p=>p.map((x,i)=>i===ci?e.target.value:x))} placeholder={`Criterio ${ci+1}`} style={{...iS,marginBottom:0,flex:1,fontSize:11}}/>
              {criterios.length>1&&<button onClick={()=>setCriterios(p=>p.filter((_,i)=>i!==ci))} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:16,flexShrink:0}}>×</button>}
            </div>
          ))}
          <button onClick={()=>setCriterios(p=>[...p,""])} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:7,color:C.muted,padding:"3px 9px",cursor:"pointer",fontSize:11,fontFamily:FONT,marginBottom:8}}>+ Agregar criterio</button>
        </div>
      )}
      ...
      {esSubida&&(
        <div>
          ...
          <div style={{background:"#5CA8E015",border:"1px solid #5CA8E033",borderRadius:9,padding:"10px 12px",fontSize:12,color:C.info}}>
            📎 Los alumnos podrán subir un archivo {evalFormato==="imagen"?"de imagen":evalFormato==="audio"?"de audio":"de video"} como respuesta.
          </div>
          <div style={{fontSize:10,color:C.muted,fontWeight:600,letterSpacing:.8,margin:"8px 0 6px"}}>CRITERIOS DE EVALUACIÓN</div>
          {criterios.map((c,ci)=>(
            <div key={ci} style={{display:"flex",gap:7,marginBottom:6}}>
              <input value={c} onChange={e=>setCriterios(p=>p.map((x,i)=>i===ci?e.target.value:x))} placeholder={`Criterio ${ci+1}`} style={{...iS,marginBottom:0,flex:1,fontSize:11}}/>
              {criterios.length>1&&<button onClick={()=>setCriterios(p=>p.filter((_,i)=>i!==ci))} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:16,flexShrink:0}}>×</button>}
      ...
      <div style={{display:"flex",gap:8,marginTop:8}}>
        <button onClick={guardar} disabled={saving||!evalTitulo.trim()}
          style={{background:tipoColor[evalTipo]||C.accent,border:"none",borderRadius:9,color:"#0D0D0D",padding:"9px 20px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT,flex:1,opacity:!evalTitulo.trim()?0.5:1}}>
