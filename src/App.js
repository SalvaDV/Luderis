        ...
        <input ref={autoFocus?searchInputRef:null} value={busqueda} onChange={e=>setBusqueda(e.target.value)}
          onKeyDown={e=>{
            ...
            if(e.key==="Enter"&&busqueda.trim()){
              setModoVista("resultados");
              // Mantener foco en el input después del re-render
            ...
            <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
              ...
              <div style={{display:"flex",gap:12,paddingBottom:6}} className="cl-cats-row">
                {cats.map(cat=>{
                  ...
                  const shortLabel=cat.label.length>14?cat.label.split(" ").slice(0,2).join(" "):cat.label;
                  return(
                    <button key={cat.label} onClick={()=>{setFiltroMateria(cat.label);setModoVista("resultados");}}
                      style={{flexShrink:0,width:130,borderRadius:14,overflow:"hidden",border:"none",cursor:"pointer",fontFamily:FONT,padding:0,background:"transparent",transition:"transform .2s",textAlign:"left"}}
                      onMouseEnter={e=>e.currentTarget.style.transform="translateY(-4px) scale(1.02)"}
                      onMouseLeave={e=>e.currentTarget.style.transform="none"}>
                      {/* Área visual con gradiente + emoji grande */}
                      <div style={{height:90,background:data.grad,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",borderRadius:"14px 14px 0 0",overflow:"hidden"}}>
                        {/* Orbe decorativo */}
                        ...
                        <span style={{fontSize:44,filter:"drop-shadow(0 2px 8px rgba(0,0,0,.2))",lineHeight:1,position:"relative",zIndex:1}}>{data.emoji}</span>
                        {/* Badge con cantidad */}
                        {cat.count>0&&<span style={{position:"absolute",bottom:6,right:8,background:"rgba(0,0,0,.35)",color:"#fff",borderRadius:20,fontSize:10,fontWeight:700,padding:"2px 7px",backdropFilter:"blur(4px)"}}>{cat.count}</span>}
                      </div>
                      {/* Label */}
                      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderTop:"none",borderRadius:"0 0 14px 14px",padding:"9px 10px"}}>
                        <div style={{fontWeight:700,color:C.text,fontSize:12,lineHeight:1.3,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{shortLabel}</div>
                        {cat.label!==shortLabel&&<div style={{fontSize:10,color:C.muted,marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cat.label.split(" ").slice(2).join(" ")}</div>}
                      </div>
                    </button>
                  );
                })}
