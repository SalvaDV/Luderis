...
function PostCard({post,session,onOpenChat,onOpenDetail,onOpenPerfil,avgPub,countPub,avgUser,yaOferte,fueRechazado,isFav,favId,onFavChange}){
  const nombre=post.autor_nombre||sb.getDisplayName(post.autor_email)||post.autor_safeDisplayName(null,email)||"Usuario";
...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil,onOpenCurso}){
  ...
  const filtered=posts.filter(p=>{
    ...
    if(busquedaDebounced&&!p.titulo?.toLowerCase().includes(q)&&!p.descripcion?.toLowerCase().includes(q)&&!(p.autor_nombre||p.autor_safeDisplayName(null,email)||"").toLowerCase().includes(q))return false;
  ...
  const handleClickEliminar=async()=>{
    if(post.tipo==="busqueda"){
      try{
        ...
        setOfertaAceptadaInfo(aceptada?{nombre:aceptada.ofertante_nombre||aceptada.ofertante_safeDisplayName(null,email),email:aceptada.ofertante_email}:null);
      ...
      <div key={ins.id} style={{background:C.card,border:`1px solid ${tieneNotif?C.accent:C.border}`,borderRadius:14,padding:"14px 18px",display:"flex",gap:13,alignItems:"center",flexWrap:"wrap",transition:"border-color .15s"}}
        ...
        <div onClick={()=>{marcarNotifPubLeida(p.id);onOpenCurso(p);}} style={{display:"flex",gap:12,alignItems:"center",flex:1,minWidth:0,cursor:"pointer"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:12,color:C.muted,marginBottom:3}}>{p.materia} · {p.autor_nombre||p.autor_safeDisplayName(null,email)}</div>
        ...
        <button onClick={()=>onOpenChat({id:p.id,autor_email:p.autor_email,titulo:p.titulo,autor_nombre:p.autor_nombre||p.autor_safeDisplayName(null,email)})}
        ...
        <div style={{marginTop:inscripciones.length>0||ayudantePubs.length>0?28:0}}>
          ...
          <div style={{display:"grid",gap:9}}>
            {clasesAcordadas.map(o=>{
              ...
              const otroN=soyDoc?(o.busqueda_autor_nombre||o.busqueda_autor_safeDisplayName(null,email)):(o.ofertante_nombre||o.ofertante_safeDisplayName(null,email));
              return(
...
function ReseñasSeccion({post,session,inscripcion,esMio}){
  ...
  return(<>
    ...
    {puedeResena&&(<div style={{marginTop:12}}>
      ...
      <textarea value={reseña} onChange={e=>setReseña(e.target.value)} placeholder="Dejá tu reseña..." style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,padding:"9px 12px",color:C.text,fontSize:13,outline:"none",resize:"vertical",minHeight:65,boxSizing:"border-box",fontFamily:FONT}}/>
      <ErrMsg msg={err}/>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                    <span style={{fontWeight:600,color:C.text,fontSize:12}}>
                      {p.autor_nombre||p.autor_safeDisplayName(null,email)}
                        ...
                        <div style={{flex:1,background:C.card,borderRadius:8,padding:"7px 10px"}}>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
                            <span style={{fontWeight:600,color:C.text,fontSize:11}}>
                              {r.autor_nombre||r.autor_safeDisplayName(null,email)}
...
function CertificadoBtn({post,session,inscripcion}){
  ...
  const generar=()=>{
    ...
    try{
      ...
      // Nombre del alumno
      const nombre=session.user.user_metadata?.display_name||session.user.safeDisplayName(null,email)||"Alumno";
      ctx.fillStyle="#F5C842";
      ...
      // Docente
      const docente=post.autor_nombre||post.autor_safeDisplayName(null,email)||"Docente";
  ...
  return(
    <div  ref={pageRef} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      <div style={{position:"sticky",top:0,zIndex:10,background:C.sidebar,borderBottom:`1px solid ${C.border}`,padding:"10px 14px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        ...
        <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:C.text,fontSize:15,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{post.titulo}</div><div style={{fontSize:11,color:C.muted}}>{post.materia} · {post.autor_nombre||post.autor_safeDisplayName(null,email)}</div></div>
...
function DetailModal({post,session,onClose,onChat,onOpenCurso,onOpenPerfil}){
  ...
  const nombre=post.autor_nombre||sb.getDisplayName(post.autor_email)||post.autor_safeDisplayName(null,email)||"Usuario";const esMio=post.autor_email===session.user.email;
    ...
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{display:"flex",gap:0,marginBottom:16,borderBottom:`2px solid ${C.border}`,background:C.surface,borderRadius:"10px 10px 0 0",padding:"0 4px"}}>
        {[
          ...
          {id:"ofertas",label:"Actividad",count:(totalOfertas+misOfertasEnv.length)||null},
      ...
      {tabCuenta==="publicaciones"&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div style={{fontSize:13,color:C.muted}}>{pubs.filter(p=>p.tipo==="oferta").length} oferta{pubs.filter(p=>p.tipo==="oferta").length!==1?"s":""} · {pubs.filter(p=>p.tipo==="busqueda").length} búsqueda{pubs.filter(p=>p.tipo==="busqueda").length!==1?"s":""}</div>
            <Btn onClick={onNew} style={{padding:"7px 18px",fontSize:13,borderRadius:20}}>+ Nueva publicación</Btn>
          </div>
          {loading?<Spinner/>:pubs.length===0?(
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:7}}>
              ...
              <div>
                <div style={{color:C.text,fontSize:12,fontWeight:600}}>{oferta.busqueda_autor_nombre||oferta.busqueda_autor_safeDisplayName(null,email)}</div>
