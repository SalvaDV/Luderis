...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil}){
  ...
  const [filtroDurMin,setFiltroDurMin]=useState(0); // semanas
  const [filtroDurMax,setFiltroDurMax]=useState(0);
  ...
  const cargar=useCallback(async()=>{
    ...
    try{
      ...
      if(durs.length){setFiltroDurMax(Math.max(...durs)+1);}
  ...
  const filtered=posts.filter(p=>{
    ...
    if(filtroDurMax>0&&filtroDurMin>0&&p.fecha_inicio&&p.fecha_fin){
  ...
  const selS={width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,padding:"7px 10px",color:C.text,fontSize:12,outline:"none",fontFamily:FONT,cursor:"pointer",boxSizing:"border-box"};
  const FLabel=({children})=><div style={{fontSize:10,fontWeight:700,letterSpacing:1.1,color:C.muted,textTransform:"uppercase",marginBottom:7}}>{children}</div>;
  const FChip=({label,active,onClick})=>(
    <button onClick={onClick} style={{padding:"5px 11px",borderRadius:20,fontSize:11,fontWeight:active?700:400,cursor:"pointer",fontFamily:FONT,background:active?C.accent:C.surface,color:active?"#0D0D0D":C.muted,border:`1px solid ${active?C.accent:C.border}`,marginBottom:5,marginRight:5}}>
      {label}
    </button>
  );
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{position:"relative",marginBottom:10}}>
        <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar por tema, materia o docente..." style={{width:"100%",background:C.surface,border:`1px solid ${busqueda?C.accent:C.border}`,borderRadius:11,padding:"10px 34px 10px 13px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:FONT,transition:"border-color .15s"}}/>
      ...
      <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
        {/* ── PANEL LATERAL ── */}
        {panelOpen&&(
          <div style={{width:215,flexShrink:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 14px",position:"sticky",top:16,maxHeight:"calc(100vh-120px)",overflowY:"auto"}}>
            ...
            {/* Fecha de inicio */}
            <div style={{marginBottom:16}}>
              <FLabel>Fecha de inicio</FLabel>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                <div>
                  <div style={{fontSize:10,color:C.muted,marginBottom:3}}>Desde</div>
                  <input type="date" value={filtroFechaDesde} onChange={e=>setFiltroFechaDesde(e.target.value)} style={{...selS,colorScheme:"dark"}}/>
                </div>
                <div>
                  <div style={{fontSize:10,color:C.muted,marginBottom:3}}>Hasta</div>
                  <input type="date" value={filtroFechaHasta} onChange={e=>setFiltroFechaHasta(e.target.value)} style={{...selS,colorScheme:"dark"}} min={filtroFechaDesde||undefined}/>
                </div>
            ...
            {clasesAcordadas.map(o=>{
              ...
              return(
                <div key={o.id} onClick={()=>setEspacioActivo(o)}
                  ...
                  <div style={{width:44,height:44,borderRadius:11,background:"#4ECB7115",border:"1px solid #4ECB7133",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0,color:C.success,fontWeight:700}}>
                    {soyDoc?"✦":"◈"}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
...
// ─── PRICE SLIDER — rango dinámico ───────────────────────────────────────────
function PriceSlider({min,max,valMin,valMax,onChangeMin,onChangeMax}){
  if(max<=0||max<=min)return null;
  const pct=(v)=>((v-min)/(max-min))*100;
  const slS={WebkitAppearance:"none",appearance:"none",width:"100%",height:4,borderRadius:2,outline:"none",cursor:"pointer",background:"transparent",position:"absolute",top:0,pointerEvents:"all"};
  return(
    <div>
      <style>{`.ps::-webkit-slider-thumb{-webkit-appearance:none;width:15px;height:15px;border-radius:50%;background:${C.accent};cursor:pointer;border:2px solid #111;box-shadow:0 0 0 1px ${C.accent}44}.ps::-moz-range-thumb{width:15px;height:15px;border-radius:50%;background:${C.accent};cursor:pointer;border:2px solid #111}`}</style>
      <div style={{position:"relative",height:20,marginBottom:4}}>
        ...
        <input type="range" className="ps" min={min} max={max} value={valMin} onChange={e=>onChangeMin(Math.min(+e.target.value,valMax-1))} style={slS}/>
        <input type="range" className="ps" min={min} max={max} value={valMax} onChange={e=>onChangeMax(Math.max(+e.target.value,valMin+1))} style={slS}/>
      </div>
...
// ─── ESPACIO CLASE MODAL — pantalla completa para oferta aceptada ─────────────
function EspacioClaseModal({oferta,session,onClose}){
  ...
  const removeC=async(id)=>{
    try{await sb.deleteContenido(id,session.access_token);setContenido(p=>p.filter(x=>x.id!==id));}catch(e){alert(e.message);}
  };
  ...
  return(
    <div ref={pageRef} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"24px 20px"}}>
          ...
          <div style={{display:"flex",gap:14,alignItems:"center"}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <span style={{fontSize:11,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"3px 9px",color:C.muted}}>{soyDocente?"Sos el docente":"Sos el alumno"}</span>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px 20px",marginBottom:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div>
              ...
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>{soyDocente?"Subí el material para tu alumno":"Material compartido por el docente"}</div>
            </div>
          ...
          {soyDocente&&showAdd&&(
            <div style={{background:"#111",border:`1px solid ${C.border}`,borderRadius:12,padding:"14px",marginBottom:14}}>
              <div style={{display:"flex",gap:5,marginBottom:9,flexWrap:"wrap"}}>
                {Object.entries(TM).map(([v,m])=>(
                  <button key={v} onClick={()=>setNuevoTipo(v)} style={{padding:"5px 10px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:FONT,background:nuevoTipo===v?C.accent:C.surface,color:nuevoTipo===v?"#0D0D0D":C.muted,border:`1px solid ${nuevoTipo===v?"transparent":C.border}`}}>{m.l}</button>
                ))}
              </div>
          ...
          {loading?<Spinner/>:contenido.length===0?(
            <div style={{textAlign:"center",padding:"36px 0",color:C.muted}}>
              ...
              <div style={{fontSize:13}}>{soyDocente?"Usá + Agregar para subir material.":"El docente aún no subió material."}</div>
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {contenido.map((item,i)=>{
                const m=TM[item.tipo]||{ic:"·",col:C.text};
                return(
                  <div key={item.id} style={{background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 15px"}}>
                    <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                      <div style={{width:32,height:32,borderRadius:8,background:C.card,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0,color:m.col,fontWeight:700,border:`1px solid ${C.border}`}}>{m.ic}</div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontWeight:600,color:m.col,fontSize:13,marginBottom:3}}>{i+1}. {item.titulo}</div>
                        ...
                        {(item.tipo==="video"||item.tipo==="link"||item.tipo==="archivo")&&item.url&&<a href={item.url} target="_blank" rel="noreferrer" style={{color:C.info,fontSize:12,textDecoration:"none",wordBreak:"break-all"}}>{item.tipo==="video"?"▶ Ver video":item.tipo==="archivo"?"↓ Abrir":"↗ Abrir link"}</a>}
                      </div>
        ...
        {/* Chat privado */}
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden"}}>
          <EspacioChat ofertaId={oferta.id} miEmail={miEmail} miId={session.user.id} otroEmail={otroEmail} otroNombre={otroNombre} session={session}/>
        </div>
      </div>
    </div>
  );
...
// ─── ESPACIO CHAT — chat privado dentro del espacio de clase ─────────────────
function EspacioChat({ofertaId,miEmail,miId,otroEmail,otroNombre,session}){
  ...
  useEffect(()=>{cargar();const t=setInterval(cargar,7000);return()=>clearInterval(t);},[cargar]);
  useEffect(()=>{if(bottomRef.current)bottomRef.current.scrollIntoView({behavior:"smooth"});},[msgs]);
  const enviar=async()=>{
    if(!texto.trim())return;setSending(true);
    const txt=texto.trim();setTexto("");
    try{
      // de_usuario es NOT NULL — usar session.user.id
      await sb.insertMensaje({publicacion_id:ofertaId,de_usuario:miId,para_usuario:null,de_nombre:miEmail,para_nombre:otroEmail,texto:txt,leido:false},session.access_token);
      await cargar();
    }catch(e){alert(e.message);setTexto(txt);}
    finally{setSending(false);}
  };
  return(
    <div>
      <div style={{padding:"14px 18px",borderBottom:`1px solid ${C.border}`,fontWeight:700,color:C.text,fontSize:13,display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{padding:"12px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8}}>
        ...
        <button onClick={enviar} disabled={sending||!texto.trim()} style={{background:C.accent,border:"none",borderRadius:10,color:"#0D0D0D",padding:"9px 16px",cursor:"pointer",fontWeight:700,fontSize:13,fontFamily:FONT,opacity:!texto.trim()||sending?0.5:1}}>→</button>
      </div>
    </div>
  );
}
