...
// ─── BÚSQUEDA CON IA ──────────────────────────────────────────────────────────
const SUGERENCIAS_DEFAULT=["Clases de guitarra para principiantes","Inglés conversacional avanzado","Matemáticas para ingresantes universitarios","Yoga y meditación online","Programación en Python desde cero","Cocina italiana presencial en Buenos Aires"];
function BusquedaIA({onBuscar,iaLoading,onClose,session}){
  const [query,setQuery]=useState("");
  const [sugerencias,setSugerencias]=useState(SUGERENCIAS_DEFAULT);
  const inputRef=useRef(null);
  useEffect(()=>{
    setTimeout(()=>inputRef.current?.focus(),80);
    // Cargar búsquedas recientes del usuario para mostrar como sugerencias
    if(session?.user?.id){
      sb.getBusquedasRecientes(session.user.id,session.access_token).then(rows=>{
        if(rows&&rows.length>0){
          // Deduplicar y tomar los últimos términos únicos
          const uniq=[...new Set(rows.map(r=>r.termino).filter(Boolean))].slice(0,6);
          setSugerencias(uniq.length>=3?uniq:[...uniq,...SUGERENCIAS_DEFAULT].slice(0,6));
        }
      }).catch(()=>{});
    }
  },[]);// eslint-disable-line
  const handleBuscar=()=>{if(!query.trim()||iaLoading)return;onBuscar(query);onClose();};
  const tieneRecientes=sugerencias!==SUGERENCIAS_DEFAULT;
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"80px 16px 16px",fontFamily:FONT,backdropFilter:"blur(4px)",animation:"fadeIn .15s ease"}} onClick={onClose}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,width:"min(600px,98vw)",boxShadow:"0 24px 64px rgba(0,0,0,.3)",overflow:"hidden",animation:"fadeUp .2s ease"}} onClick={e=>e.stopPropagation()}>
        ...
        {/* Sugerencias — dinámicas si el usuario tiene historial */}
        <div style={{padding:"16px 20px"}}>
          <div style={{fontSize:11,fontWeight:600,color:C.muted,letterSpacing:.5,marginBottom:12}}>{tieneRecientes?"TUS BÚSQUEDAS RECIENTES":"SUGERENCIAS"}</div>
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            {sugerencias.map(s=>(
              <button key={s} onClick={()=>{setQuery(s);setTimeout(()=>inputRef.current?.focus(),50);}}
                ...
                onMouseEnter={e=>e.currentTarget.style.background=C.bg}
                onMouseLeave={e=>e.currentTarget.style.background="none"}>
                <span style={{fontSize:16,flexShrink:0}}>{tieneRecientes?"🕐":"🔍"}</span>
                <span>{s}</span>
              </button>
            ))}
          </div>
...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil,onOpenCurso}){
  ...
  // Función que lanza búsqueda y popula iaResults
  // Estrategia: 1) RPC DB con pg_trgm+unaccent (rápido, sin tokens IA)
  //             2) Si el usuario menciona ubicación o la DB no da resultados → IA como fallback
  const buscarConIA=useCallback(async(q)=>{
    ...
    // Guardar búsqueda en historial (silencioso)
    if(session?.user?.id){
      sb.guardarBusqueda({usuario_id:session.user.id,termino:q.trim()},session.access_token).catch(()=>{});
    }

    try{
      // ── Paso 1: búsqueda en DB con pg_trgm + unaccent ──────────────────────
      // Resuelve acentos ("matematica" → "Ciencia y Matemática"), errores de tipeo, etc.
      const dbResults=await sb.buscarPublicaciones({texto:q,limite:40},session?.access_token).catch(()=>[]);

      if(dbResults&&dbResults.length>0&&!mencionaUbic){
        // Resultados directos de la DB — sin costo de IA
        setIaResults(dbResults.map(p=>p.id));
        setIaExplanation(`${dbResults.length} resultado${dbResults.length!==1?"s":""} encontrado${dbResults.length!==1?"s":""}`);
        // Agregar publicaciones al mapa de posts si no estaban cargadas
        setPosts(prev=>{
          const existIds=new Set(prev.map(p=>p.id));
          const nuevos=dbResults.filter(p=>!existIds.has(p.id));
          return nuevos.length>0?[...prev,...nuevos]:prev;
        });
        return;
      }

      // ── Paso 2: fallback IA (cuando menciona ubicación o DB sin resultados) ─
      const cityCtx=(mencionaUbic||userCity)?`\nCiudad del usuario: ${userCity||"desconocida"}. ${mencionaUbic?"Priorizá presenciales en o cerca de "+userCity+".":"Considerá la ciudad del usuario."}`:"";
      // Combinar resultados DB (si los hay) con posts en memoria para dar contexto a la IA
      const poolIds=new Set(dbResults?.map(p=>p.id)||[]);
      const pool=[
        ...(dbResults||[]),
        ...posts.filter(p=>!poolIds.has(p.id)),
      ].slice(0,80);
      const postsCtx=pool.map(p=>({id:p.id,titulo:p.titulo,materia:p.materia,descripcion:(p.descripcion||"").slice(0,100),tipo:p.tipo,precio:p.precio,modalidad:p.modalidad,ubicacion:p.ubicacion||""}));
      const raw=await sb.callIA(
      ...
      setIaExplanation(scored.length>0?"Resultado de búsqueda":"Sin resultados.");
  ...
  return(<>
    ...
    {showBusquedaIA&&<BusquedaIA onBuscar={buscarConIA} iaLoading={iaLoading} onClose={()=>setShowBusquedaIA(false)} session={session}/>}
  </>);
}
