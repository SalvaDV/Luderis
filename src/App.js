  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:FONT}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:22,padding:"36px 32px",width:"min(420px,92vw)",textAlign:"center"}}>
        <div style={{width:64,height:64,borderRadius:18,background:C.accentDim,border:`1px solid #F5C84233`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,margin:"0 auto 14px"}}>🎓</div>
        <h2 style={{fontFamily:"Georgia,serif",color:C.text,fontSize:24,margin:"0 0 5px"}}>Classe<span style={{color:C.accent}}>Link</span></h2>
        <p style={{color:C.muted,marginBottom:26,fontSize:13,lineHeight:1.5}}>Conectá con profesores y estudiantes</p>
        ...
        {mode==="forgot"?(<>
          ...
          <ErrMsg msg={err}/>{ok&&<div style={{color:C.success,fontSize:13,marginBottom:8}}>{ok}</div>}
          <Btn onClick={handle} disabled={loading} style={{width:"100%",padding:"13px",fontSize:14,marginBottom:12,borderRadius:12,letterSpacing:.3}}>{loading?"Cargando...":mode==="login"?"Entrar →":"Crear cuenta →"}</Btn>
...
function Sidebar({page,setPage,session,onLogout,onNewPost,unreadCount,ofertasCount,notifCount,mobile,open,onClose}){
  ...
  const nav=[
    ...
    {id:"chats",icon:"◻",label:"Chats",badge:unreadCount},
    {id:"favoritos",icon:"◇",label:"Guardados"},
    {id:"inscripciones",icon:"◈",label:"Inscripciones",badge:notifCount},
  ...
  const inner=(
    <div style={{width:224,height:"100%",background:C.sidebar,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <nav style={{padding:"10px 8px",flex:1}}>
        ...
        <Btn onClick={()=>{onNewPost();if(mobile)onClose();}} style={{width:"100%",padding:"9px 10px",fontSize:12,borderRadius:9,letterSpacing:.3}}>+ Publicar</Btn>
      </nav>
...
function PostCard({post,session,onOpenChat,onOpenDetail,onOpenPerfil,avgPub,countPub,avgUser}){
  ...
  const accentBar=post.tipo==="oferta"?(post.modo==="curso"?C.success:C.info):C.accent;
  return(
    <div onClick={()=>onOpenDetail(post)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px 18px",cursor:"pointer",transition:"border-color .2s,box-shadow .2s",position:"relative",overflow:"hidden",fontFamily:FONT}}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=C.accent;e.currentTarget.style.boxShadow=`0 4px 24px #0006`;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.boxShadow="none";}}>
      <div style={{position:"absolute",top:0,left:0,width:"100%",height:3,background:accentBar}}/>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:8}}>
        <div style={{display:"flex",gap:9,alignItems:"center",minWidth:0}}>
          <button onClick={e=>{e.stopPropagation();onOpenPerfil(post.autor_email);}} style={{background:"none",border:"none",cursor:"pointer",padding:0,flexShrink:0}}><Avatar letra={nombre[0]}/></button>
          <div style={{minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
              <button onClick={e=>{e.stopPropagation();onOpenPerfil(post.autor_email);}} style={{fontWeight:700,color:C.text,fontSize:13,background:"none",border:"none",cursor:"pointer",fontFamily:FONT,padding:0}} onMouseEnter={e=>{e.currentTarget.style.color=C.accent;}} onMouseLeave={e=>{e.currentTarget.style.color=C.text;}}>{nombre}</button>
      ...
      <p style={{color:C.muted,fontSize:12,lineHeight:1.6,margin:"0 0 10px"}}>{post.descripcion?.slice(0,120)}{post.descripcion?.length>120?"...":""}</p>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:10}}>
        ...
        {post.tipo==="oferta"&&post.sinc&&post.modo==="curso"&&<span style={{fontSize:11,padding:"3px 9px",borderRadius:20,background:C.surface,color:C.muted,border:`1px solid ${C.border}`}}>{post.sinc==="sinc"?"Sincrónico":"Asincrónico"}</span>}
        {post.precio&&<span style={{fontSize:12,color:C.accent,fontWeight:700,background:C.accentDim,borderRadius:7,padding:"3px 10px",marginLeft:"auto"}}>{fmtPrice(post.precio)}{post.precio_tipo&&post.modo!=="curso"?` /${post.precio_tipo}`:""}</span>}
      </div>
      <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center",borderTop:`1px solid ${C.border}`,paddingTop:10,marginTop:2}} onClick={e=>e.stopPropagation()}>
        {post.fecha_inicio&&<span style={{fontSize:11,color:C.muted}}>{fmt(post.fecha_inicio)}</span>}
  ...
  return(
    <div style={{fontFamily:FONT}}>
      <div style={{marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"flex-end",gap:12,flexWrap:"wrap"}}>
        <div>
          <h1 style={{fontSize:"clamp(18px,4vw,26px)",fontWeight:700,margin:"0 0 3px",lineHeight:1.2}}>Explorar <span style={{color:C.accent}}>clases</span></h1>
          <p style={{color:C.muted,fontSize:12,margin:0}}>Publicaciones activas de la comunidad</p>
        </div>
      </div>
      <div style={{position:"relative",marginBottom:11}}>
        <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar por tema, materia o docente..." style={{width:"100%",background:C.surface,border:`1px solid ${busqueda?C.accent:C.border}`,borderRadius:12,padding:"10px 14px 10px 38px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",fontFamily:FONT,transition:"border-color .2s"}}/>
        <span style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",color:C.muted,fontSize:14,pointerEvents:"none"}}>⊞</span>
        {busqueda&&<button onClick={()=>setBusqueda("")} style={{position:"absolute",right:11,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.muted,fontSize:16,cursor:"pointer",padding:0,lineHeight:1}}>×</button>}
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:18,flexWrap:"wrap"}}>
        {[{label:"Total",val:filtered.length,c:C.text},{label:"Búsquedas",val:filtered.filter(p=>p.tipo==="busqueda").length,c:C.accent},{label:"Cursos",val:filtered.filter(p=>p.tipo==="oferta"&&p.modo==="curso").length,c:C.success},{label:"Particulares",val:filtered.filter(p=>p.tipo==="oferta"&&p.modo==="particular").length,c:C.info}].map(s=>(<div key={s.label} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:11,padding:"9px 14px",flex:1,minWidth:60,textAlign:"center"}}><div style={{fontSize:18,fontWeight:700,color:s.c,lineHeight:1}}>{s.val}</div><div style={{color:C.muted,fontSize:10,marginTop:3,letterSpacing:.3}}>{s.label}</div></div>))}
      </div>
      {loading?<Spinner/>:filtered.length===0?(<div style={{textAlign:"center",color:C.muted,padding:"46px 0",fontSize:13}}>{posts.length===0?"Sé el primero en publicar.":"Sin resultados para esa búsqueda."}</div>):(
...
function MyPostCard({post,session,onEdit,onToggle,onDelete,onOpenCurso,toggling,ofertasPendientes,inscriptos}){
  ...
  return(
    <div style={{background:finalizado?"#0f0f0f":C.card,border:`1px solid ${ofertasPendientes>0?C.accent:finalizado?C.border:C.border}`,borderRadius:14,padding:"14px 16px",position:"relative",overflow:"hidden",fontFamily:FONT,opacity:finalizado?.75:1}}>
    ...
    return(
      <div key={ins.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 18px",display:"flex",gap:13,alignItems:"center",flexWrap:"wrap",transition:"border-color .15s"}}
        ...
        <div onClick={()=>onOpenCurso(p)} style={{display:"flex",gap:12,alignItems:"center",flex:1,minWidth:0,cursor:"pointer"}}>
          <div style={{width:44,height:44,borderRadius:11,background:finalizado?"#4ECB7115":p.modo==="particular"?"#5CA8E015":C.accentDim,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,border:`1px solid ${finalizado?"#4ECB7133":p.modo==="particular"?"#5CA8E033":"#F5C84222"}`}}>
            {finalizado?"✓":p.modo==="particular"?"◈":"◉"}
          </div>
          <div style={{flex:1,minWidth:0}}>
  ...
  return(
    <div style={{fontFamily:FONT}}>
      <div style={{marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
        <div>
          <h2 style={{fontSize:20,color:C.text,margin:"0 0 2px",fontWeight:700}}>Mis inscripciones</h2>
          {!loading&&<p style={{color:C.muted,fontSize:12,margin:0}}>{inscripciones.length+ayudantePubs.length} en total</p>}
        </div>
      </div>
...
function CursoPage({post,session,onClose,onUpdatePost}){
  ...
  if(!tieneAcceso&&!loading&&!inscLoading){
    ...
    return(<div style={{position:"fixed",inset:0,background:C.bg,zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:FONT,gap:16,padding:24}}>
      ...
      <div style={{width:56,height:56,borderRadius:16,background:C.surface,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,marginBottom:8}}>{cerrado?"○":"◈"}</div>
      <h2 style={{color:C.text,fontSize:20,fontWeight:700,margin:0}}>{cerrado?"Inscripciones cerradas":"Contenido privado"}</h2>
      <p style={{color:C.muted,fontSize:14,textAlign:"center",maxWidth:360,margin:0}}>{cerrado?"El docente cerró las inscripciones para este curso.":"Inscribite gratis para acceder a todo el contenido."}</p>
  ...
  return(
    <div  ref={pageRef} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
      ...
      {needsValoracion&&(
        <div style={{background:C.accentDim,border:`1px solid #F5C84233`,margin:"16px 20px",borderRadius:14,padding:"14px 18px",display:"flex",alignItems:"center",gap:12}}>
          <span style={{fontSize:22}}>★</span>
          <div style={{flex:1}}><div style={{color:C.accent,fontWeight:700,fontSize:14,marginBottom:2}}>Las clases finalizaron</div><div style={{color:C.muted,fontSize:12}}>Podés dejar tu valoración abajo.</div></div>
    ...
    <Modal onClose={onClose} width="min(660px,97vw)">
      <div style={{padding:"19px 21px"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:13}}>
          <div style={{background:C.accentDim,border:"1px solid #F5C84222",borderRadius:9,padding:"7px 11px"}}>
            <div style={{fontSize:9,color:C.muted,letterSpacing:1,marginBottom:3,textTransform:"uppercase"}}>Materia</div>
            <span style={{color:C.accent,fontSize:12,fontWeight:700}}>{post.materia}</span>
          </div>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:9,padding:"7px 11px",flex:1,minWidth:120}}>
            <div style={{fontSize:9,color:C.muted,letterSpacing:1,marginBottom:3,textTransform:"uppercase"}}>Valoración pub.</div>
            {loading?<Spinner small/>:<StarRating val={avgPub} count={reseñas.length}/>}
          </div>
          {post.created_at&&<div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:9,padding:"7px 11px"}}>
            <div style={{fontSize:9,color:C.muted,letterSpacing:1,marginBottom:3,textTransform:"uppercase"}}>Publicado</div>
            <span style={{color:C.muted,fontSize:12}}>{fmt(post.created_at)}</span>
          </div>}
        </div>
    ...
    <div style={{fontFamily:FONT}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:18,padding:"22px 24px",marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:9,flexWrap:"wrap"}}>
          ...
          <Chip label="INSCRIPTOS" val={`${Object.values(inscritosMap).reduce((a,b)=>a+b,0)}`}/>
        </div>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h3 style={{color:C.text,fontSize:16,fontWeight:700,margin:0}}>Credenciales</h3>
      ...
      {ofertasAceptRec.filter(o=>!o.finalizada_cuenta).length>0&&(
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
            <h3 style={{color:C.text,fontSize:16,fontWeight:700,margin:0}}>Clases acordadas</h3>
            <span style={{background:"#4ECB7115",color:C.success,border:"1px solid #4ECB7133",borderRadius:20,fontSize:11,fontWeight:700,padding:"2px 9px"}}>{ofertasAceptRec.filter(o=>!o.finalizada_cuenta).length}</span>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
      ...
      {(()=>{
        ...
        return(
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
              <h3 style={{color:C.text,fontSize:16,fontWeight:700,margin:0}}>Mis ofertas enviadas</h3>
              <span style={{background:C.surface,color:C.muted,border:`1px solid ${C.border}`,borderRadius:20,fontSize:11,fontWeight:600,padding:"2px 9px"}}>{mostrar.length}</span>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>(
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
          <h3 style={{color:C.text,fontSize:16,fontWeight:700,margin:0}}>Reseñas recibidas</h3>
          {reseñas.length>0&&<span style={{background:C.accentDim,color:C.accent,border:"1px solid #F5C84233",borderRadius:20,fontSize:11,fontWeight:700,padding:"2px 9px"}}>{reseñas.length}</span>}
        </div>
