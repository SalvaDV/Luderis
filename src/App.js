...
function OfertasRecibidasModal({post,session,onClose,onContactar}){
  ...
  useEffect(()=>{
    cargar().then(()=>{
      // Marcar todas las ofertas de esta búsqueda como leídas al abrir el modal
      sb.getOfertasSobre(post.id,session.access_token).then(all=>{
  ...
  return(
    <div style={{fontFamily:FONT}}>
      ...
      {(()=>{
        ...
        const marcarVista=(o)=>{const k=vistaId(o);if(!vistas.includes(k)){const nv=[...vistas,k];localStorage.setItem(vistasKey,JSON.stringify(nv));if(onRefreshOfertas)onRefreshOfertas();}};
        // Marcar automáticamente al abrir Mi cuenta todo lo que ya está visible
        // (el badge se limpia en onClearBadge al montar, pero marcarVista sincroniza localStorage)
        return(
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"18px 20px",marginBottom:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {mostrar.map(o=>{
                const esNueva=!vistas.includes(vistaId(o))&&(o.estado==="aceptada"||o.estado==="rechazada"||(o.estado==="pendiente"&&o.contraoferta_precio&&o.contraoferta_de==="alumno"));
                const diasEspera=o.estado==="pendiente"&&o.created_at?Math.floor((Date.now()-new Date(o.created_at))/86400000):null;
                return(
                <div key={o.id} onClick={()=>{if(esNueva)marcarVista(o);}} style={{background:esNueva&&o.estado==="aceptada"?"#4ECB7108":esNueva&&o.estado==="rechazada"?"#E05C5C08":esNueva?"#C85CE008":C.surface,border:`1px solid ${esNueva&&o.estado==="aceptada"?"#4ECB71":esNueva&&o.estado==="rechazada"?"#E05C5C":esNueva?"#C85CE0":o.estado==="rechazada"?"#E05C5C33":o.estado==="aceptada"?"#4ECB7133":C.border}`,borderRadius:12,padding:"11px 14px",display:"flex",gap:10,alignItems:"center",position:"relative",overflow:"hidden",cursor:esNueva?"pointer":"default"}}>
...
export default function App(){
  ...
  const refreshUnread=useCallback(()=>{
    ...
    Promise.all([
      ...
      let vistas=[];try{vistas=JSON.parse(localStorage.getItem(vistasKey)||"[]");}catch{}
      const nuevas=(misOfertas||[]).filter(o=>{
        if(vistas.includes(o.id+"_"+o.estado+(o.contraoferta_de||"")))return false;
        // Aceptada no vista
        if(o.estado==="aceptada")return true;
        // Rechazada no vista
        if(o.estado==="rechazada")return true;
        // Contraoferta del alumno no vista (yo soy docente, alumno=busqueda_autor)
        if(o.estado==="pendiente"&&o.contraoferta_precio&&o.contraoferta_de==="alumno")return true;
        return false;
      }).length;
      setNotifCount(nfs.length);setNotifs(nfs);
      setOfertasAceptadasNuevas(nuevas);
    }).catch(()=>{});
  },[session]);
