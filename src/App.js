...
function InscripcionesPage({session,onOpenCurso,onOpenChat,onMarkNotifsRead}){
  ...
  const marcarNotifPubLeida=async(pubId)=>{
    ...
    try{
      await sb.marcarNotifsTipoLeidas(session.user.email,["valorar_curso","nuevo_ayudante","busqueda_acordada"],session.access_token);
  ...
  return(
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        {tipo==="oferta"&&(<>
          ...
          {modo==="curso"&&(<div style={{borderTop:`1px solid ${C.border}`,paddingTop:10}}>
            ...
            {sinc==="sinc"&&(<>
              ...
              {clasesSinc.map((c,i)=>{
                const horaInvalida=c.hora_fin&&c.hora_inicio&&c.hora_fin<=c.hora_inicio;
        ...
        <ErrMsg msg={err}/>
        {sinc==="sinc"&&clasesSinc.some(c=>c.hora_fin&&c.hora_inicio&&c.hora_fin<=c.hora_inicio)&&(
        ...
        <Btn onClick={guardar} disabled={saving||clasesSinc.some(c=>c.hora_fin&&c.hora_inicio&&c.hora_fin<=c.hora_inicio)} style={{width:"100%",padding:"10px",fontSize:13,marginTop:11,borderRadius:11}}>{saving?"Guardando...":editing?"Guardar cambios":"Publicar"}</Btn>
      </div>
    </Modal>
  );
}
...
export default function App(){
  ...
  const refreshUnread=useCallback(()=>{
    ...
    Promise.all([
      ...
      const notifsInsc=(nfs||[]).filter(n=>["valorar_curso","nuevo_ayudante","busqueda_acordada"].includes(n.tipo));
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"66px 13px 22px":"30px 28px",minHeight:"100vh",maxWidth:`calc(100vw - ${SW}px)`}}>
        <div style={{maxWidth:820,margin:"0 auto"}}>
          ...
          {page==="inscripciones"&&<InscripcionesPage session={session} onOpenCurso={setCursoPost} onOpenChat={openChat} onMarkNotifsRead={()=>{sb.marcarNotifsTipoLeidas(session.user.email,["valorar_curso","nuevo_ayudante","busqueda_acordada"],session.access_token).then(refreshUnread).catch(()=>{});}}/>}
