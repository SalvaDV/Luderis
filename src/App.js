...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil,onOpenCurso}){
  ...
  // Cuando hay búsqueda activa o filtros → modo resultados
  useEffect(()=>{
    if(busquedaDebounced||filtroTipo!=="all"||filtroModo!=="all"||filtroModalidad!=="all"||filtroMateria||filtroUbicacion){
      setModoVista("resultados");
    }
  },[busquedaDebounced,filtroTipo,filtroModo,filtroModalidad,filtroMateria,filtroUbicacion]);
  ...
  const filtered=posts.filter(p=>{
    ...
    if(filtroUbicacion&&!(p.ubicacion||"").toLowerCase().includes(filtroUbicacion.toLowerCase()))return false;
  ...
  const SearchBar=({autoFocus})=>(
    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
      <div style={{position:"relative",flex:1,minWidth:200}}>
        ...
        <input ref={autoFocus?searchInputRef:null} value={busqueda} onChange={e=>setBusqueda(e.target.value)}
          onKeyDown={e=>{if(e.key==="Escape")setBusqueda("");if(e.key==="Enter"&&busqueda.trim())setModoVista("resultados");}}
  ...
  // Categorías visuales para el home
  const cats=categorias.length>0
    ?categorias.slice(0,8).map(c=>({icon:c.icono||"📚",label:c.nombre,val:c.nombre}))
    :MATERIAS.slice(0,8).map(m=>({icon:"📚",label:m,val:m}));
  ...
  return(<>
    <div style={{fontFamily:FONT}}>
        ...
        <div>
          ...
          <div style={{background:`linear-gradient(135deg,${LUD.dark} 0%,${LUD.blue} 60%,${LUD.teal} 100%)`,borderRadius:16,padding:"36px 28px",marginBottom:24,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{position:"relative",zIndex:1}}>
              ...
              <p style={{color:"rgba(255,255,255,.75)",fontSize:14,margin:"0 0 20px",lineHeight:1.5}}>
          ...
          {/* Categorías horizontales scrolleables */}
          <div style={{marginBottom:24}}>
            <div style={{fontWeight:700,color:C.text,fontSize:15,marginBottom:12}}>Explorar por materia</div>
            <div style={{display:"flex",gap:10,overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none",paddingBottom:4}}>
              <style>{`.cl-cats::-webkit-scrollbar{display:none}`}</style>
              <div style={{display:"flex",gap:10}} className="cl-cats">
                {cats.map(cat=>(
                  <button key={cat.val} onClick={()=>{setFiltroMateria(cat.val);setModoVista("resultados");}}
                    style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",cursor:"pointer",fontFamily:FONT,flexShrink:0,minWidth:82,transition:"all .18s"}}
                    ...
                    <span style={{fontSize:11,fontWeight:600,color:C.text,textAlign:"center",lineHeight:1.2,maxWidth:70}}>{cat.label}</span>
                  </button>
                ))}
                <button onClick={()=>setModoVista("resultados")}
                  style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,background:C.accentDim,border:`1px solid ${C.accent}33`,borderRadius:12,padding:"14px 16px",cursor:"pointer",fontFamily:FONT,flexShrink:0,minWidth:82}}> 
                  <span style={{fontSize:26}}>→</span>
                  <span style={{fontSize:11,fontWeight:600,color:C.accent,textAlign:"center"}}>Ver todo</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:24}}>
            {[
              ...
              {icon:"📍",title:"Presencial",desc:"Cerca tuyo",onClick:()=>{setFiltroModalidad("presencial");setModoVista("resultados");}},
            ].map(item=>(
      ...
      {modoVista==="resultados"&&(
        <div>
          {/* Barra de búsqueda + botón volver */}
              ...
              <div style={{position:"fixed",top:0,right:0,bottom:0,width:"min(320px,90vw)",background:C.surface,zIndex:151,boxShadow:"-4px 0 24px rgba(0,0,0,.12)",overflowY:"auto",WebkitOverflowScrolling:"touch",animation:"slideInRight .2s ease",display:"flex",flexDirection:"column",fontFamily:FONT}}>
                ...
                <div style={{padding:"16px 20px",flex:1}}>
                  ...
                  <div style={{marginBottom:16}}><FL ch="Materia"/>
                    {categorias.length>0?(<select value={filtroMateria} onChange={e=>setFiltroMateria(e.target.value)} style={selS}><option value="">Todas</option>{categorias.map(c=><option key={c.slug} value={c.nombre}>{c.icono} {c.nombre}</option>)}</select>)
          ...
          {loading?<Spinner/>:filtered.length===0?(
            ...
            <div>
              ...
              {filtered.length>pagina*PAGE_SIZE&&(
                <div ref={sentinelRef} style={{height:44,display:"flex",alignItems:"center",justifyContent:"center",marginTop:8}}><Spinner small/></div>
              )}
