...
function PostFormModal({session,postToEdit,onClose,onSave}) {
  ...
  const guardar = async()=>{
    ...
    try{
      const nombre = session.user.email.split("@")[0];
      const data = {tipo,materia,titulo,descripcion,autor_email:session.user.email,autor_nombre:nombre,autor_avatar:nombre[0].toUpperCase(),activo:true};
      if(tipo==="oferta"){
  ...
  const iStyle = {width:"100%",background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 14px",color:C.text,fontSize:14,outline:"none",boxSizing:"border-box",marginBottom:12,fontFamily:"'Open Sans',sans-serif"};

  return (
    <div style={{position:"fixed",inset:0,background:"#000b",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Open Sans',sans-serif"}}>
