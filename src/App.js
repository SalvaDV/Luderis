...
// ─── BÚSQUEDA CON IA ──────────────────────────────────────────────────────────
function BusquedaIA({posts,session,userCity,onOpenDetail,onClose}){
...
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil,onOpenCurso}){
  ...
  return(<>
        ...
        <div>
          ...
          <div style={{background:C.surface,borderRadius:12,padding:"14px 16px",marginBottom:14,border:`1px solid ${C.border}`,boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
              ...
              <div style={{flex:1}}>{searchBarJSX}</div>
            </div>
    ...
    {showBusquedaIA&&<BusquedaIA posts={posts} session={session} userCity={userCity} onOpenDetail={onOpenDetail} onClose={()=>setShowBusquedaIA(false)}/>}
  </>);
}
