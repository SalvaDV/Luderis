...
function VerificacionIA({titulo,materia,descripcion,onVerificado,onEstadoChange,token}){
  ...
  const tituloDebounced=useDebounce(titulo,8000);
  const descripcionDebounced=useDebounce(descripcion,8000);
  useEffect(()=>{
    ...
    <Modal onClose={onClose}>
      <div style={{padding:"20px"}}>
        ...
        {tipo==="oferta"&&(<>
          ...
          {titulo&&materia&&descripcion.length>=30&&!verificado&&<VerificacionIA titulo={titulo} materia={materia} descripcion={descripcion} onVerificado={(v)=>{setVerificado(v!==false);setVerificacionPendiente(false);}} onEstadoChange={(e)=>setVerificacionPendiente(e==="cargando")} token={session?.access_token}/>}
