...
function VerificacionIA({titulo,materia,descripcion,onVerificado,onEstadoChange,token}){
  ...
  return(<div style={{background:C.accentDim,border:`1px solid #F5C84233`,borderRadius:10,padding:12,marginTop:8}}>
    ...
    {estado==="cargando"?<div style={{color:C.muted,fontSize:12,display:"flex",alignItems:"center",gap:6}}><Spinner small/>Generando...</div>:(<>
      ...
      {estado==="error"&&feedback&&<div style={{color:C.danger,fontSize:12,marginBottom:7,background:"#E05C5C15",borderRadius:7,padding:"7px 10px",lineHeight:1.5}}>{feedback}</div>}
      <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <button onClick={()=>{onVerificado(false);}} style={{background:"none",border:"none",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:FONT}}>Omitir (sin verificar)</button>
      </div>
    </>)}
  </div>);
}
