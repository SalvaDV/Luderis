...
const StatusBadge=({activo,finalizado,pendiente})=>{
  ...
  if(pendiente||(!activo&&pendiente!==false&&pendiente!==undefined))return<span style={{fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,background:"#F5C84218",color:C.accent,border:"1px solid #F5C84233",fontFamily:FONT}}>⏳ Validación pendiente</span>;
...
function EvaluacionCard({ev,post,session,esMio,inscripciones,inscripcion,onDelete}){
  ...
  return(
    <div style={{background:C.card,border:`1px solid ${tipoColor[ev.tipo]||C.border}22`,borderRadius:12,overflow:"hidden",opacity:(ev.tipo==="final"&&!ev.activo&&!esMio)?0.6:1}}>
      {/* Header */}
      <div onClick={()=>setExpanded(v=>!v)} style={{padding:"12px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:6,marginTop:3,flexWrap:"wrap"}}>
            ...
            {!esMio&&!entrega&&ev.tipo==="diagnostico"&&inscripcion&&<span style={{fontSize:10,background:"#F5C84215",color:C.accent,borderRadius:20,padding:"1px 7px",border:"1px solid #F5C84233"}}>⚡ Pendiente</span>}
            {!esMio&&ev.tipo==="final"&&!ev.activo&&<span style={{fontSize:10,background:C.surface,color:C.muted,borderRadius:20,padding:"1px 7px",border:`1px solid ${C.border}`}}>🔒 No habilitado</span>}
            {esMio&&ev.tipo==="final"&&!ev.activo&&<span style={{fontSize:10,background:"#E0955C15",color:C.warn,borderRadius:20,padding:"1px 7px",border:"1px solid #E0955C33"}}>No habilitado</span>}
        ...
        {esMio&&ev.tipo==="final"&&(
          <button onClick={async(e)=>{e.stopPropagation();try{await sb.updateEvaluacion(ev.id,{activo:!ev.activo},session.access_token);ev.activo=!ev.activo;setExpanded(false);setTimeout(()=>setExpanded(v=>v),1);}catch(err){alert(err.message);}}}
            style={{background:ev.activo?"#E05C5C15":"#4ECB7115",border:`1px solid ${ev.activo?"#E05C5C33":"#4ECB7133"}`,borderRadius:8,color:ev.activo?C.danger:C.success,padding:"4px 10px",cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:FONT,flexShrink:0}}>
            {ev.activo?"Deshabilitar":"Habilitar"}
          </button>
        )}
      ...
      {expanded&&(
        <div style={{borderTop:`1px solid ${C.border}`,padding:"14px 16px"}}>
          {/* Examen final bloqueado para alumnos */}
          {!esMio&&ev.tipo==="final"&&!ev.activo&&(
            <div style={{textAlign:"center",padding:"20px 0",color:C.muted,fontSize:13}}>
              🔒 El docente aún no habilitó el examen final.
            </div>
          )}
          {/* Vista alumno — rendir evaluación */}
          {!esMio&&inscripcion&&!entrega&&(ev.tipo!=="final"||ev.activo)&&(
            <div>
          ...
          {esMio&&ev.tipo==="checkpoint"&&<button onClick={onDelete} style={{background:"none",border:`1px solid ${C.danger}`,borderRadius:8,color:C.danger,padding:"5px 12px",cursor:"pointer",fontSize:11,fontFamily:FONT,marginTop:8}}>Eliminar evaluación</button>}
          {esMio&&ev.tipo!=="checkpoint"&&<div style={{fontSize:11,color:C.muted,marginTop:8,padding:"5px 0"}}>Los exámenes de diagnóstico y finales no se pueden eliminar, solo editar.</div>}
        </div>
      )}
    </div>
  );
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"16px 20px"}}>
        ...
        <div>
          ...
          {tabActivo==="evaluaciones"&&<div style={{marginBottom:18}}>
            ...
            {(!(post.activo===false&&post.estado_validacion==="pendiente"))&&(
        ...
        return(<div key={p.id} style={{background:C.surface,border:`1px solid ${cnt>0?C.accent:C.border}`,borderRadius:14,padding:"14px 16px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:6}}><Tag tipo={p.tipo}/><StatusBadge activo={p.activo!==false} finalizado={!!p.finalizado} pendiente={p.activo===false&&p.estado_validacion==="pendiente"}/></div>
