...
function AuthScreen({onLogin}){
  ...
  if(showTerminos)return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:FONT}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:22,padding:"32px 28px",width:"min(620px,95vw)",maxHeight:"88vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <h2 style={{color:C.text,fontSize:18,fontWeight:700,margin:0}}>Términos y Condiciones</h2>
          <button onClick={()=>setShowTerminos(false)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:8,color:C.muted,padding:"5px 12px",cursor:"pointer",fontSize:13,fontFamily:FONT}}>← Volver</button>
        </div>
        {[
          ...
          <div key={titulo} style={{marginBottom:18}}>
            <div style={{fontWeight:700,color:C.text,fontSize:13,marginBottom:5}}>{titulo}</div>
            <p style={{color:C.muted,fontSize:12,lineHeight:1.7,margin:0}}>{texto}</p>
          </div>
        ))}
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:16,marginTop:8}}>
          <p style={{color:C.muted,fontSize:11,textAlign:"center",margin:0}}>Última actualización: {new Date().toLocaleDateString("es-AR",{month:"long",year:"numeric"})}</p>
        </div>
        <button onClick={()=>{setAceptoTerminos(true);setShowTerminos(false);}} style={{width:"100%",marginTop:16,background:C.accent,border:"none",borderRadius:10,color:"#0D0D0D",padding:"11px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT}}>Acepto los Términos y Condiciones ✓</button>
      </div>
    </div>
  );
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:FONT}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:22,padding:"36px 32px",width:"min(420px,92vw)",textAlign:"center"}}>
        ...
        {mode==="forgot"?(<>
          ...
          {mode==="register"&&<Input type="password" placeholder="Repetir contraseña" value={pass2} onChange={e=>setPass2(e.target.value)} style={{marginBottom:12}}/>}
          {mode==="register"&&(
            <div style={{display:"flex",alignItems:"flex-start",gap:9,marginBottom:12,textAlign:"left"}}>
              <button onClick={()=>setAceptoTerminos(v=>!v)}
                style={{width:18,height:18,flexShrink:0,borderRadius:4,border:`2px solid ${aceptoTerminos?C.accent:C.border}`,background:aceptoTerminos?C.accent:"transparent",cursor:"pointer",padding:0,display:"flex",alignItems:"center",justifyContent:"center",marginTop:1}}>
                {aceptoTerminos&&<span style={{color:"#0D0D0D",fontSize:11,fontWeight:700,lineHeight:1}}>✓</span>}
              </button>
              <span style={{fontSize:12,color:C.muted,lineHeight:1.5}}>
                Acepto los{" "}
                <button onClick={()=>setShowTerminos(true)} style={{background:"none",border:"none",color:C.accent,cursor:"pointer",fontSize:12,fontFamily:FONT,padding:0,textDecoration:"underline",fontWeight:600}}>Términos y Condiciones</button>
                {" "}de ClasseLink
              </span>
            </div>
          )}
          <ErrMsg msg={err}/>{ok&&<div style={{color:C.success,fontSize:13,marginBottom:8}}>{ok}</div>}
          <Btn onClick={handle} disabled={loading||(mode==="register"&&!aceptoTerminos)} style={{width:"100%",padding:"12px",fontSize:14,marginBottom:12,borderRadius:12,opacity:(mode==="register"&&!aceptoTerminos)?0.5:1}}>{loading?"...":mode==="login"?"Entrar →":"Crear cuenta →"}</Btn>
