...
function OfertasRecibidasModal({post,session,onClose,onContactar}){
  ...
  const responder=async(o,estado)=>{
    ...
    try{
      ...
      if(estado==="aceptada"){
        // Abrir modal de acuerdo antes de ir al chat
              ...
              <div key={o.id} style={{background:C.surface,border:"1px solid #4ECB7133",borderRadius:12,padding:"13px 15px"}}>
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0}}>
                    ...
                    <button onClick={()=>setAcuerdoModal(o)} style={{background:"#4ECB7115",border:"1px solid #4ECB7133",borderRadius:8,color:C.success,padding:"5px 10px",cursor:"pointer",fontSize:11,fontFamily:FONT,fontWeight:700}}>{o.acuerdo_confirmado?"✓ Ver acuerdo":"📋 Acuerdo"}</button>
...
function AcuerdoModal({oferta,session,onClose,onConfirmado}){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      const otroEmail=soyDocente?oferta.busqueda_autor_email:oferta.ofertante_email;
      sb.insertNotificacion({usuario_id:null,alumno_email:otroEmail,tipo:"acuerdo_confirmado",publicacion_id:oferta.busqueda_id||oferta.id,pub_titulo:oferta.busqueda_titulo||"Clase",leida:false},session.access_token).catch(()=>{});
      setConfirmado(true);
      ...
      <div style={{padding:"22px 24px"}}>
        ...
        {confirmado?(
          ...
          <div>
            ...
            <Btn onClick={guardar} disabled={saving} style={{width:"100%",padding:"11px",fontSize:14}}>
