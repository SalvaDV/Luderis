    ...
    <div style={{width:224,height:"100%",background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <div style={{padding:"10px 12px",borderTop:`1px solid ${C.border}`,display:"flex",flexDirection:"column",gap:6}}>
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          <button onClick={onForceRender} style={{display:"none"}}/>
        )}
        {["salvadordevedia@gmail.com"].includes(session.user.email)&&(
          <button onClick={()=>{if(mobile)onClose();window.__openAdmin&&window.__openAdmin();}} style={{background:"linear-gradient(135deg,#7B3FBE,#1A6ED8)",border:"none",borderRadius:10,color:"#fff",padding:"10px 14px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:FONT,width:"100%",marginBottom:8,display:"flex",alignItems:"center",gap:8}}>⚙️ Panel Admin</button>
        )}
...
export default function App(){
  ...
  onSave={(newPub,meta)=>{
    ...
    if(newPub&&(meta?.esCursoNuevo||meta?.esParticularNuevo)){
      ...
      {showAdmin&&<AdminPage session={session} onClose={()=>setShowAdmin(false)}/>}
      <ScrollToTopBtn/>
      <ChatBotWidget/>
      <ToastContainer/>
