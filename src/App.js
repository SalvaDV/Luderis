...
function OfertarBtn({post,session}){
  ...
  const enviar=async()=>{
    ...
    try{
      const payload={busqueda_id:post.id,busqueda_autor_email:post.autor_email,ofertante_email:session.user.email,ofertante_nombre:session.user.email.split("@")[0],precio:precio?parseFloat(precio):null,precio_tipo:tipo,mensaje:msg,leida:false};
...
function ChatCurso({post,session,ayudantes=[]}){
  ...
  const cargar=useCallback(async()=>{
    try{
      // Traer mensajes del grupo: para_nombre == "__grupo__" filtramos en cliente
      const data=await sb.getMensajes(post.id,"__grupo__","__grupo__",session.access_token).catch(()=>[]);
      // getMensajes filtra por par de emails; usamos getMisChats y filtramos por pubId + para_nombre
      const allChats=await sb.getMisChats(miEmail,session.access_token).catch(()=>[]);
      const grupal=allChats.filter(m=>String(m.publicacion_id)===String(post.id)&&m.para_nombre==="__grupo__");
      // También traer mensajes donde de_nombre!==miEmail (de otros inscriptos)
      // Usamos endpoint directo filtrado por publicacion_id y para_nombre=__grupo__
      setMsgs(grupal);
      if(!didScrollRef.current&&grupal.length>0){didScrollRef.current=true;setTimeout(()=>bottomRef.current?.scrollIntoView({behavior:"smooth"}),60);}
    }finally{setLoading(false);}
  },[post.id,miEmail,session.access_token]);
  ...
  const send=async()=>{
    ...
    try{
      await sb.insertMensaje({publicacion_id:post.id,de_nombre:miEmail,para_nombre:"__grupo__",texto:txt,leido:true,pub_titulo:post.titulo},session.access_token);
      ...
      await Promise.all(todos.map(e=>sb.insertNotificacion({tipo:"chat_grupal",alumno_email:e,publicacion_id:post.id,pub_titulo:post.titulo,leida:false}).catch(()=>{})));
...
function CursoPage({post,session,onClose}){
  ...
  return(
    <div ref={el=>el&&(el.scrollTop=0)} style={{position:"fixed",inset:0,background:C.bg,zIndex:300,overflowY:"auto",fontFamily:FONT}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:7}}>
                {inscripciones.map(ins=>{
                  ...
                  return(<div key={ins.id} style={{display:"flex",alignItems:"center",gap:9,padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{color:C.text,fontSize:12,fontWeight:600}}>{ins.alumno_email.split("@")[0]}</div>
      ...
      {showFinalizar&&<FinalizarClaseModal post={post} session={session} onClose={()=>setShowFinalizar(false)} onFinalizado={()=>setLocalFinalizado(true)}/>}
      {showDenuncia&&<DenunciaModal post={post} session={session} onClose={()=>setShowDenuncia(false)}/>}
      {showCerrarInsc&&<CerrarInscModal post={post} session={session} onClose={()=>setShowCerrarInsc(false)} onCerrado={()=>setLocalCerrado(true)}/>}
    </div>
  );
}
...
function MiCuentaPage({session,onOpenDetail,onOpenCurso,onEdit,onNew,onOpenChat,onRefreshOfertas}){
  ...
  // Perfil edición
  const [editingPerfil,setEditingPerfil]=useState(false);const [displayName,setDisplayName]=useState(()=>localStorage.getItem("displayName_"+session.user.email)||"");const [savingDisplayName,setSavingDisplayName]=useState(false);
        ...
        {editingPerfil&&(
          <div style={{background:"#1a1a1a",border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              ...
              <button onClick={async()=>{setSavingDisplayName(true);localStorage.setItem("displayName_"+email,displayName);setSavingDisplayName(false);alert("Nombre actualizado. Los cambios se verán desde ahora.");}} disabled={savingDisplayName} style={{background:C.accent,border:"none",borderRadius:9,color:"#0D0D0D",padding:"8px 14px",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:FONT}}>{savingDisplayName?"...":"Guardar"}</button>
            </div>
