...
// ─── EXPLORE PAGE ─────────────────────────────────────────────────────────────
function ExplorePage({session,onOpenChat,onOpenDetail,onOpenPerfil,initialMateria="",onClearInitialMateria}){
  ...
  const [filtroSinc,setFiltroSinc]=useState("all"); const [filtroMateria,setFiltroMateria]=useState(initialMateria||"");
  ...
  useEffect(()=>{if(initialMateria){setFiltroMateria(initialMateria);setPanelOpen(true);if(onClearInitialMateria)onClearInitialMateria();}},[initialMateria]);// eslint-disable-line
...
function AyudanteBuscador({post,session,ayudantesActuales,onUpdate}){
  ...
  const agregar=async()=>{
    ...
    try{
      // Buscar el UUID del usuario por email (la columna ayudantes es uuid[])
...
// ─── ONBOARDING ───────────────────────────────────────────────────────────────
function OnboardingModal({session,onClose,onFinish}){
    ...
    {title:"¡Todo listo!",sub:"Ya podés empezar a explorar ClasseLink.",body:(
      <div style={{textAlign:"center",padding:"16px 0"}}>
        ...
        <p style={{color:C.muted,fontSize:13,lineHeight:1.8}}>
          {rol==="docente"?"A continuación vas a crear tu primera publicación.":rol==="alumno"?"Vamos a mostrarte clases relacionadas con tus materias.":"Podés explorar y publicar según lo que necesites."}<br/>
          <span style={{color:C.muted,fontSize:11}}>Tus preferencias se guardaron.</span>
        </p>
      </div>
    )},
  ];
  ...
  const finish=()=>{
    ...
    onClose();
    // Disparar acción según rol
    if(onFinish)onFinish({rol,materias});
  };
  return(
...
export default function App(){
  ...
  const [onboardingMateria,setOnboardingMateria]=useState("");
  useEffect(()=>{const fn=()=>setIsMobile(window.innerWidth<768);window.addEventListener("resize",fn);return()=>window.removeEventListener("resize",fn);},[]);
  const handleFinishOnboarding=({rol,materias})=>{
    if(rol==="docente"){setTimeout(()=>{setEditPost(null);setShowForm(true);},200);}
    else if(materias&&materias.length>0){setOnboardingMateria(materias[0]);}
  };
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,color:C.text,display:"flex"}}>
      ...
      <main style={{marginLeft:SW,flex:1,padding:isMobile?"66px 13px 22px":"30px 28px",minHeight:"100vh",maxWidth:`calc(100vw - ${SW}px)`}}>
        <div style={{maxWidth:820,margin:"0 auto"}}>
          {page==="explore"&&<ExplorePage session={session} onOpenChat={openChat} onOpenDetail={setDetailPost} onOpenPerfil={setPerfilEmail} initialMateria={onboardingMateria} onClearInitialMateria={()=>setOnboardingMateria("")}/>}
      ...
      {showOnboarding&&session&&<OnboardingModal session={session} onClose={()=>setShowOnboarding(false)} onFinish={handleFinishOnboarding}/>}
      <ChatBotWidget/>
    </div>
  );
}
