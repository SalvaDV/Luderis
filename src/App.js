    ...
    <div style={{background:C.card,border:`1px solid ${ofertasPendientes>0?C.accent:C.border}`,borderRadius:14,padding:"14px 16px",position:"relative",overflow:"hidden",fontFamily:FONT}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:10}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          {post.tipo==="oferta"&&inscriptos!==undefined&&<div style={{marginTop:4,fontSize:12,color:C.muted,display:"flex",gap:10}}><span><span style={{color:C.text,fontWeight:600}}>{inscriptos}</span> inscripto{inscriptos!==1?"s":""}</span>{post.vistas>0&&<span style={{color:C.muted}}>👁 {post.vistas} vista{post.vistas!==1?"s":""}</span>}</div></div>}
