...
function DocentesDestacados({posts,onOpenPerfil,session}){
  ...
  posts.filter(p=>p.tipo==="oferta"&&p.activo).forEach(p=>{
    ...
    if(!docenteMap[email])docenteMap[email]={
      email,nombre:p.autor_nombre||email.split("@")[0],avatar:p.autor_avatar_url||null,color:p.autor_avatar_color||null,
    ...
    if(p.autor_avatar_color&&!d.color)d.color=p.autor_avatar_color;
    d.pubs++;
          ...
          {top.map((d)=>{
            ...
            <button key={d.email} onClick={()=>onOpenPerfil(d.email)}
              ...
              <div style={{position:"relative",marginBottom:12}}>
                <Avatar letra={d.nombre[0]} size={56} radius={16} img={d.avatar||undefined} color={d.color||undefined}/>
...
function AgendaPage({session,onOpenCurso,onGoExplore}){
  ...
  // Registro manual. La sección de arriba se apoya en el calendario, que solo
