        ...
        {tipo==="oferta"&&(<>
          ...
          {modo==="particular"&&precio&&(
            <div style={{marginTop:8,borderTop:`1px solid ${C.border}`,paddingTop:10}}>
              ...
              {paquetes.length===0&&(
                <div style={{color:C.muted,fontSize:12,textAlign:"center",padding:"12px 0",background:C.bg,borderRadius:8}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {paquetes.map((pq,i)=>{
                  ...
                  const precioFinal=pq.descuento>0
                    ?(precioNum*(pq.clases||1)*(1-(pq.descuento||0)/100))
                    :(pq.precio_total||(precioNum*(pq.clases||1)));
                  return(
                    <div key={i} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}}>
                      ...
                      <div style={{display:"flex",gap:8"}}>
                        <div style={{flex:1}}>
                          <div style={{fontSize:10,color:C.muted,fontWeight:600,marginBottom:3}}>CANTIDAD DE CLASES</div>
                      ...
                      {(pq.clases>0&&precioNum>0)&&(
                        <div style={{fontSize:11,color:C.success,fontWeight:600,background:C.success+"10",borderRadius:6,padding:"4px 10px",alignSelf:"flex-start"}}>
                          ...
                          {pq.descuento>0&&<span style={{color:C.muted,marginLeft:6}}>(vs ${(precioNum*(pq.clases||1)).toLocaleString("es-AR")} sin descuento)</span>}
                        </div>
                      )}
                    </div>
                  );
